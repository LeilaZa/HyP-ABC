...
#students = pd.read_csv('/Users/leila/Desktop/HPO/Ready.csv')
...
EvalNum=0
MaxEval= 5000#FOOD_NUMBER+ iter*(2*FOOD_NUMBER) + iter
#RUN_TIME=2
...
def init(i): #creating random food source for each row based on X=random*(Upper-Lower)
        ...
        for key in search:
                if search[key]["type"] == "int" or search[key]["type"] == "ctg":
                    ...
                    foods[i][j]=np.round((random.uniform(search[list(search.keys())[j]]["range"][0],
                                               search[list(search.keys())[j]]["range"][1])),5)
                    print(str(key) +": "+str(foods[i][j]))
                j=j+1
...
#***Baseline***
#------------------------------------------------
#X_train, X_test, y_train, y_test = train_test_split(X, y, cv=cv,shuffle=False,random_state=100)

model_x= XGBClassifier (objective='binary:logistic', use_label_encoder=False, random_state=99,
                        n_estimators=341 , learning_rate= 0.0234, 
                         max_depth=10 ,subsample=0.378, 
                         colsample_bytree=0.9187 , verbosity=0, n_jobs=-1)
res_x=np.mean(cross_val_score(model_x, X, y, cv=cv, n_jobs=-1,scoring="accuracy"))

print("F(x):" + str(res_x))

#8879775452481246= 88.80
#n_estimators=341 , learning_rate= 0.0234, max_depth=10 ,subsample=0.3786 ,colsample_bytree=0.9187 , verbosity=0, n_jobs=-1)
